...
def send_email_user_mentions(
    ...
    if not (comment := Comment.objects.filter(id=comment_id).first()):
        return
    ...
    for comment_text in comment_text_list:
        if (
            ...
            and comment_text.strip("@").strip(",") not in recipients
    ...
    context["url"] = settings.DOMAIN_NAME if subject else ""
...
def send_email_user_delete(
    ...
    if not user_email:
        return
    context = {"message": "deleted", "deleted_by": deleted_by, "email": user_email}
    recipients = [user_email]
    html_content = render_to_string(
...
def resend_activation_link_to_user(
    ...
    if user_obj:
        ...
        recipients = [context["complete_url"], user_email]
        html_content = render_to_string("user_status_in.html", context=context)
        if recipients:
            subject = "Welcome to Django CRM"
            msg = EmailMessage(subject, html_content, from_email=settings.DEFAULT_FROM_EMAIL, to=recipients)
